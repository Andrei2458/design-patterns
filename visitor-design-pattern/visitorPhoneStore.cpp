...
class MobileVisitor{
    public:
        virtual void visitBasicPhone(BasicPhone const* element) const = 0;
        virtual void visitSmartPhone(SmartPhone const* element) const = 0;
};
...
class SmartPhone : public MobileComponent {
    public:
        ...
        uint16_t getSmartPhoneRam() const {
            return this->ramSize;
        }
    private:
        uint16_t batteryCapacity;
...
class MobileVisitorImpl: public MobileVisitor {
    public:
        virtual ~MobileVisitorImpl() {}
        void visitBasicPhone(BasicPhone const* element) const {
        ...
        void visitSmartPhone(SmartPhone const* element) const {
            cout << "Mobile Phone Specifications:\n";
            cout << "Phone Type: Smart Phone\nBattery Capacity: " << element->getSmartPhoneBatteryCapacity()
                 << "mAh\nRam Size: " << element->getSmartPhoneRam() << "GB\n";
        }
};
...
int main() {
    ...
    auto iterator = begin(mobilePhones);
    while(iterator!=end(mobilePhones)) {
